...
class HumanPlayer(Player):
    ...
    def get_move(self, game):
        ...
        while not valid_square:
            ...
            # it to an integer, and if it's not, then we sayits invalid
            #if that spot is not available on the board, we also say its inalid
            try:
                val = int(square)
...
class GeniusComputerPlayer(Player):
    ...
    def minimax(self, state, player):
        ...
        other_player = 'O' if player == 'X' else 'X' # the other plaer.. so whatever letter is NOT
